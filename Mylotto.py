...
def random_pick():
    ...
    if num1 in lotto_numbers and num2 in lotto_numbers and num3 in lotto_numbers:
        ...
        print("The winning numbers are " + str(
            lotto_numbers) + ". You got one of the numbers! \nCongratulations " + Name + " you won R200! ")
    else:
        print("The winning numbers are " + str(
            lotto_numbers) + ". You didn't get any of the numbers! \nSorry " + Name + " you lost ")
...
def account_creation_prompts():
    ...
    print("Congrats " + str(name) + " " + str(
        surname) + "! Your account has been successfully created. \nYour user ID is: " + str(
        ID) + ". Now you can try logging in")
...
def login_prompts():
    ...
    while user_login_password not in login_password:
        ...
        greeting()
        initial_decision = input("Do you want to play lotto today? (Yes/No): ")
        prompts = ["yes", "no"]
        if initial_decision.lower() == "yes":
            lotto_decision()
            restart()
            print("\nDone by : StixTheMiner")
        elif initial_decision.lower() == "no":
            print("Thanks " + Name + " . Good bye")
            exit()
        else:
            print("Invalid response! Good bye " + Name)
            exit()

def welcome_prompts():
    login = input(
        "Hello and welcome to 'MyLotto'. Do you want to create account or login? \nPlease reply with 'create' or 'login': ")
